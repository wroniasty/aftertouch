...
TEST_CASE("free kick take resumes open play") {
    ...
    s.sides[0].control.normal_fire = 1; // hold = driven free kick
...
TEST_CASE("throw-in take clears hide ball") {
    ...
    s.sides[0].control.controlled_slot = 1;
    s.sides[0].control.normal_fire = 1; // hold = long throw
    ...
    CHECK(GetPl(s) == GameStatePl::InProgress);
    CHECK(s.globals.hide_ball == 0);
    CHECK(s.ball.speed == kRestartThrowSpeed);
    CHECK(static_cast<PlayerState>(s.players[1].player_state) ==
          PlayerState::Normal);
}

TEST_CASE("throw-in ignores fire_this_frame until classified") {
    MatchState s{};
    BeginRestart(s, GameState::ThrowInCentreLeft, 70, 400, 0x1F, 4, 1);
    s.sides[0].control.controlled_slot = 1;
    ...
    s.players[1].pos.x = Fix::FromInt(67);
    s.players[1].pos.y = Fix::FromInt(400);
    CHECK_FALSE(ApplyRestartTake(s, 0));
