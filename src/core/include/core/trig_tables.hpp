...
// GENERATED by tools/gen_trig.py — do not edit by hand.
// See doc/implementation/A2-determinism-primitives.md section 2.3.

namespace at {

// kAngleTangent[dy][dx]: round(32*y/x) in the first quadrant,
// folded so T[y][x] = 64 - T[x][y] when x < y. Origin is -1.
inline constexpr std::array<std::array<int8_t, 32>, 32> kAngleTangent = {{
...
// kSineCosineTable[i] ~= round(32767 * sin(2π i / 256)).
// Consumed as (v >> 8) * speed; the >>8 lane matches the reference
// table for every entry even where ±1 transcription drift exists.
inline constexpr std::array<int16_t, 256> kSineCosineTable = {{
