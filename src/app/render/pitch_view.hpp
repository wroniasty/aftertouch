...
// Ball-follow window (~half the dead-ball box). Tunable.
inline constexpr int16_t kFollowWinW = 280;
inline constexpr int16_t kFollowWinH = 175;
...
inline ScreenPos PitchToScreen(int16_t pitch_x, int16_t pitch_y,
    ...
    const float nx =
        (static_cast<float>(pitch_x) - static_cast<float>(view.min_x)) * scale;
    const float ny =
        (static_cast<float>(pitch_y) - static_cast<float>(view.min_y)) * scale;
    ScreenPos s;
