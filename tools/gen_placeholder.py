...
def write_player(path: Path, count: int, fill: int, fingerprint_tag: bytes) -> int:
    entries = []
...
def write_ball(path: Path) -> int:
    # Four rotation frames then the shadow, matching the imported bank's shape so the
    # renderer indexes both sources identically.
    entries = []
    for i in range(BALL_FRAMES):
        px = bytearray([2] * (BALL_W * BALL_H))
        px[i % (BALL_W * BALL_H)] = 3
        entries.append((BALL_W, BALL_H, 1, 3, bytes(px)))
    entries.append((BALL_W, BALL_H, 1, 3, bytes([8] * (BALL_W * BALL_H))))
    fp = fnv(b"ball")
    for *_, px in entries:
        fp = fnv(px, fp)
...
def write_numbers(path: Path) -> int:
    # Shirt numbers 1..16: 3×5 up to nine, 5×5 from ten, as in the original bank.
    entries = []
    for n in range(1, NUMBER_COUNT + 1):
        ...
        for y in range(5):
            px[y * w + (w - 1)] = 2
        entries.append((w, 5, w // 2, 5, bytes(px)))
    fp = fnv(b"numbers")
    for *_, px in entries:
        fp = fnv(px, fp)
    path.write_bytes(build_pack(KIND_SPRITES, entries, SOURCE_PLACEHOLDER, fp))
...
def main() -> int:
    ...
    for name, fill in (("kit_vstripe", 10), ("kit_hstripe", 11), ("kit_sleeves", 12)):
        packs.append(
            (f"{name}.atp", write_player(args.out / f"{name}.atp", PLAYER_FRAMES, fill, name.encode()))
        )
    packs.append(
        ("keepers.atp", write_player(args.out / "keepers.atp", KEEPER_FRAMES, 6, b"keepers"))
    )
    packs.append(("ball.atp", write_ball(args.out / "ball.atp")))
    packs.append(("numbers.atp", write_numbers(args.out / "numbers.atp")))
    ...
    (args.out / "README.md").write_text(
        ...
        "packs: three 101×12×15 kit geometries, 58 keeper frames, four 4×4 ball frames "
        "plus a shadow, 16 shirt numbers, 16×16 tiles, 42×55 matrix. Not original "
        "artwork.\n",
        encoding="utf-8",
    )
